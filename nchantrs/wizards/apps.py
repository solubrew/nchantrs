...
class NchantdApplicationStartupWizard(NchantdWizard):
        ...
        if self.create_paths_config(paths):
            if self.create_paths_application(paths):
                ...
                if self.create_paths_shortcut(paths):
                    self.app.model.store.cache_app_install('install_succeeded', ['directories_created', {'paths': paths}])
        state = True
        return (paths, state)
    ...
    def create_shortcut(self) -> Any:
        ...
        if self.os_type == 'linux':
            ...
            with open(self.shortcut_path, 'w') as desktop_file:
                data = {'<[application_name]>': self.slug, '<[icon_path]>': self.icon_path, '<[application_path]>': self.application_path}
    ...
    def reject(self) -> None:
        """"""
        reply = pyqt.QMessageBox.question(self, 'Exit Installer', 'Are you sure you want to cancel the installation?', pyqt.QMessageBox.Yes | pyqt.QMessageBox.No)
