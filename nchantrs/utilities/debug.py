...
class DataSourceDumper:
    ...
    def get_table_names(self) -> List[str]:
        ...
        # These are the core app tables based on models.py
        return [
            'app_action',
            'app_document_type', 
            'app_event',
    ...
    def dump_store_info(self) -> Dict[str, Any]:
        ...
        try:
            result["state"] = {
                "name": self.store.name if hasattr(self.store, 'name') else None,
                "db_path": self.store.get_file_path() if hasattr(self.store, 'get_file_path') else None,
                "instance_id": self.store.instance_id if hasattr(self.store, 'instance_id') else None,
            }
    ...
    def dump_model_state(self) -> Dict[str, Any]:
        ...
        try:
            result["state"] = {
                "user": str(self.model.user) if self.model.user else None,
                "instance": str(self.model.instance) if hasattr(self.model, 'instance') else None,
                "config": str(self.model.config) if hasattr(self.model, 'config') else None,
                "has_store": self.model.store is not None if hasattr(self.model, 'store') else False,
            }
    ...
    def dump_app_instances(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result["instances"] = [
                {"id": str(i), "name": getattr(i, 'name', 'unknown')}
                for i in instances
            ] if instances else []
    ...
    def dump_all(self) -> Dict[str, Any]:
        ...
        return {
            "store": self.dump_store_info(),
            "model": self.dump_model_state(),
            "instances": self.dump_app_instances(),
            "options": self.dump_app_options(),
            "events": self.dump_app_events(),
        }
