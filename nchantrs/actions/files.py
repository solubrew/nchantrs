...
def loadFile(self, fileName) -> None:
    ...
    if not file.open(QFile.ReadOnly | QFile.Text):
        QMessageBox.warning(self, 'Meldung', 'Cannot read file %s:\n%s.' % (fileName, file.errorString()))
        return
...
def maybeSave(self) -> None:
    if self.myeditor.document().isModified():
        ret = QMessageBox.warning(self, 'QTextEdit Meldung', 'Das Dokument wurde geändert.\nSollen die Änderungen gespeichert werden?', QMessageBox.Save | QMessageBox.Discard | QMessageBox.Cancel, defaultButton=QMessageBox.Save)
...
def saveFile(self, fileName) -> None:
    ...
    if not file.open(QFile.WriteOnly | QFile.Text):
        QMessageBox.warning(self, 'Message', 'Cannot write file %s:\n%s.' % (fileName, file.errorString()))
        return False
