...
class NchantdInstance(object):
    ...
    def to_dict(self):
        """"""
        return {
            "instance_id_txt": self.instance_id,
            "name_txt": self.name,
            "description_ltxt": self.description,
            "is_primary_bit": self.is_primary,
            "application_NCD_txt": self.application_NCD,
            "application_path_txt": self.application_path,
            "instance_path_txt": self.instance_path,
            "version_txt": self.version,
            "meta_data_dict": json.dumps(self.meta_data),
        }
...
class NchantdStore(MicroStash):
    ...
    def store_app_media_content(self, content, db="db", how="INSERT"):
        ...
        payload = [
            [
                ...
                content["page"],
                content["entry"],
                _coerce_doc_version(content.get("version")) + 1,
                content["hash"],
                content["content"],
                content["context"],
            ]
    ...
    def store_app_user(self, user, db="db", how="INSERT"):
        ...
        payload = [
            [
                user.uuid,
                text_hashing_function(user.parent.device.mac),
                user.parent.device.user,
                encode64(user.hash),
                encode64(user.salt),
                encode64(str(user.iters)),
                encode64(user.address),
                encode64(user.rsa_key),
                self.app.model.is_private,
                self.app.model.is_secure,
                self.app.model.instance.instance_id,
            ]
        ]
...
class GlainNchantdStore:
    ...
    def __init__(
        ...
        self._db = GlainDatabase(
            db_path=db_path or ":memory:", conn=connection, table_prefix=table_prefix, load_extensions=True
        )
        if table_prefix:
    ...
    def add_document(
        ...
        return self._db.add_document(
            content=content, metadata=metadata, entities=entities, summary=summary, privacy_level=privacy_level
        )
    ...
    def search_fts(
        ...
        return self._db.search_fts(
            query=query, limit=limit, metadata_filter=metadata_filter, privacy_levels=privacy_levels
        )
    ...
    def search_vector(
        ...
        return self._db.search_vector(
            query_embedding=query_embedding, limit=limit, metadata_filter=metadata_filter, privacy_levels=privacy_levels
        )
    ...
    def get_filtered_chunks(
        ...
        return self._db.get_filtered_chunks(
            metadata_filter=metadata_filter, include_embeddings=include_embeddings, privacy_levels=privacy_levels
        )
