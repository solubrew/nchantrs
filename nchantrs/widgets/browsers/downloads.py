...
# from PySide6.QtCore import QUrl, QThread, pyqtSignal
...
class NchantdDownloadManager(pyqt.QObject):
    ...
    downloadProgress = pyqt.Signal(str, int, int)  # download_id, received, total
    ...
    def handle_download_request(self, download: pyqt.QWebEngineDownloadRequest) -> str:
        ...
        if not save_path:
            download.cancel()
            return ""
        download_info = NchantdDownloadInfo(
            id=download_id,
            ...
            status=NchantdDownloadStatus.IN_PROGRESS,
        )
        # Configure download
    ...
    def _get_save_path(self, suggested_name: str) -> str:
        ...
        # For now, use default path with suggested name
    ...
    def clear_completed_downloads(self):
        ...
        completed_ids = [
            ...
            if info.status
            in [NchantdDownloadStatus.COMPLETED, NchantdDownloadStatus.FAILED, NchantdDownloadStatus.CANCELLED]
        ]
    ...
    def load_download_history(self):
        ...
        try:
            ...
            for data in history_data:
                data["status"] = NchantdDownloadStatus(data["status"])  # Convert string back to enum
...
class DownloadItemWidget(pyqt.QWidget):
    ...
    cancelRequested = pyqt.Signal(str)  # download_id
    pauseRequested = pyqt.Signal(str)  # download_id
    resumeRequested = pyqt.Signal(str)  # download_id
...
class DownloadsDialog(pyqt.QDialog):
    ...
    def setup_ui(self):
        ...
        self.downloads_layout.addStretch()  # Push items to top
    ...
    def open_file(self, download_id: str):
        ...
        if download_info and os.path.exists(download_info.save_path):
            os.startfile(download_info.save_path)  # Windows
    ...
    def clear_completed_downloads(self):
        ...
        for download_id, download_info in list(self.download_manager.downloads.items()):
            if download_info.status in [
                NchantdDownloadStatus.COMPLETED,
                NchantdDownloadStatus.FAILED,
                NchantdDownloadStatus.CANCELLED,
            ]:
                if download_id in self.download_widgets:
    ...
    def show_settings(self):
        ...
        path = pyqt.QFileDialog.getExistingDirectory(
            self, "Select Download Directory", self.download_manager.default_download_path
        )
        if path:
...
class CodecDownloadThread(pyqt.QThread):
    ...
    def run(self):
        try:
            ...
            # Set environment variable for WebEngine
...
class NchantdDownloadsManagerSigil(NchantdWidget):
    ...
    def __init__(self, parent=None, cfg=None):
        """ """
        self.parent = parent
        self.config = condor.Instruct(pxcfg).select("NchantdDownloadsManager")
        if self.parent:
            self.config.override(parent.config)
        self.config.override(cfg)
        super().__init__(self.parent, self.config)
