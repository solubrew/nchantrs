...
<(META)>:  #																	||
	docid:   #																	||
	name: Nchantrs Widgets Forms Python Excecution Document  #	||
	description: >  #															||
	expirary: <[expiration]>  #													||
	version: <[version]>  #														||
	path: <[LEXIvrs]>  #														||
...
log = True
#===============================================================================||
pxcfg = f'{here}_data_/forms.yaml'#									||assign default config
class NchantdDynamicRecordEntryForm(pyqt.QWidget):
	...
	def __init__(self, parent=None, cfg={}):
		...
		super(NchantdDynamicRecordEntry, self).__init__()
	...
	def loadFields(self):
		''' '''
		for field in self.config.dikt['fields'].keys():
			cfg = self.config.dikt['fields'][field]
	...
	def submitEntry(self, record):
		''' '''
		self.app.src.saveRecord(record)
		return self
